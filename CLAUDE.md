...
- **Composants partagés :** Le chrome commun du site (header/footer) est centralisé dans `src/components/page-chrome.tsx` et réutilisé sur les pages internes.
- **Design system :** La palette est définie dans `src/styles.css` avec les couleurs de marque : bleu nuit, corail/rouge, fond clair, ainsi qu'une esthétique “pop/neobrutaliste” avec bordures épaisses, ombres et boutons à fort contraste.
