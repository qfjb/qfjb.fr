...
- **Déploiement (prérendu statique) :** Le site n'a aucune donnée dynamique (pas de `loader`, pas de `createServerFn`). `vite.config.ts` active `prerender` : `npm run build` génère un HTML complet par route dans `dist/client/` (`index.html`, `equipe/index.html`, etc.). On déploie **uniquement `dist/client/`** en FTP à la racine du domaine (hébergement statique OVH, pas de Node). `dist/server/` est ignoré. Les chemins d'assets sont absolus (`/assets/...`) donc le site doit être servi depuis la racine du domaine. `public/.htaccess` gère le repli 404 (`ErrorDocument 404 /404.html`) et le cache ; `public/404.html` est une page statique autonome. Procédure complète (scriptée + manuelle) dans `DEPLOY.md` ; `npm run deploy` build + upload FTP de `dist/client/` (script `scripts/deploy.mjs` via `basic-ftp`, config dans `.env.deploy`).
- **Formulaire de contact :** `src/components/contact-form.tsx`, envoi via **Web3Forms** (POST `https://api.web3forms.com/submit`, pas de backend). Clé d'accès dans `VITE_WEB3FORMS_ACCESS_KEY` (`.env`, cf. `.env.example`) — clé publique par nature, inlinée dans le bundle client. Inclut honeypot `botcheck`, case de consentement RGPD (lien vers `/confidentialite`), états idle/submitting/success/error. Jamais de server function TanStack (incompatible hébergement statique).
- **Page confidentialité :** `src/routes/confidentialite.tsx` (route `/confidentialite`), liée depuis la case de consentement du formulaire et les footers. Décrit la collecte (formulaire uniquement), Web3Forms comme relais, la conservation (12 mois max) et les droits RGPD.
